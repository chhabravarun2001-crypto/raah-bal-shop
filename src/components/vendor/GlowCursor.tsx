// @ts-nocheck -- vendored third-party source, kept as-authored.
// Vendored from React Bits (https://reactbits.dev), MIT + Commons Clause.
// Copyright (c) David Haz — https://github.com/DavidHDev/react-bits
// Used as part of the RAAH | BAL storefront; not resold or redistributed
// as a standalone component per the Commons Clause condition.
"use client";
...
const GlowCursor = ({
  ...
  useEffect(() => {
    ...
    // Bound to `window`, not `container`: this is used as a full-page,
    // pointer-events:none overlay so it never blocks clicks on real content
    // underneath it — and an element with pointer-events:none never receives
    // its own pointer events, so listening on the container would go silent.
  ...
  return (
    <div ref={containerRef} className={`relative h-full w-full overflow-hidden${className ? ` ${className}` : ''}`} style={style} {...rest}>
      <canvas ref={canvasRef} className="pointer-events-none absolute inset-0 block h-full w-full select-none" style={{ mixBlendMode: blendMode }} aria-hidden="true" />
